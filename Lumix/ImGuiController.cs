...
public class ImGuiController : IDisposable
{
    ...
    public unsafe ImGuiController(GraphicsDevice gd, OutputDescription outputDescription, int width, int height)
    {
        ...
        // load custom font
        TryGetEmbeddedResourceBytes("Roboto-Regular", out var fontData);
        GCHandle pinnedArray = GCHandle.Alloc(fontData, GCHandleType.Pinned);
        IntPtr pointer = pinnedArray.AddrOfPinnedObject();

        // Roboto is loaded twice, first to merge with FontAwesome icons, second to merge wit Fontaudio icons
        ImGui.GetIO().Fonts.AddFontFromMemoryTTF(pointer, fontData.Length, 16 + 4);
        LoadIcons(14 + 4);
        ImGui.GetIO().Fonts.AddFontFromMemoryTTF(pointer, fontData.Length, 16 + 4);
        LoadAudioIcons(16 + 6);
        pinnedArray.Free();
    ...
    static unsafe void LoadIcons(float fontSize)
    {
        ...
        IconRanges[2] = 0;
        int size = IconRanges.Length * sizeof(ushort);

        fixed (ushort* range = &IconRanges[0])
        {
            FontAwesome6.IconFontRanges = Marshal.AllocHGlobal(size);
            Buffer.MemoryCopy(range, FontAwesome6.IconFontRanges.ToPointer(), size, size);
            icons_config.GlyphRanges = (IntPtr)(ushort*)FontAwesome6.IconFontRanges.ToPointer();

            byte[] fontDataBuffer = Convert.FromBase64String(FontAwesome6.IconFontData);

            fixed (byte* buffer = fontDataBuffer)
            {
                var fontPtr = ImGui.GetIO().Fonts.AddFontFromMemoryTTF(new IntPtr(buffer), fontDataBuffer.Length, fontSize, icons_config, FontAwesome6.IconFontRanges);
            }
        }
    }

    static unsafe void LoadAudioIcons(float fontSize)
    {
        ImFontConfigPtr icons_config = ImGuiNative.ImFontConfig_ImFontConfig();
        icons_config.MergeMode = true;
        icons_config.PixelSnapH = true;
        icons_config.FontDataOwnedByAtlas = false;
        icons_config.GlyphOffset = new Vector2(0, 4); // icons are pushed 4 pixels down to align with text
        ...
        ushort[] IconRanges = new ushort[3];
        IconRanges[0] = Fontaudio.IconMin;
        IconRanges[1] = Fontaudio.IconMax;
        IconRanges[2] = 0;

        fixed (ushort* range = &IconRanges[0])
        {
            Fontaudio.IconFontRanges = Marshal.AllocHGlobal(6);
            Buffer.MemoryCopy(range, Fontaudio.IconFontRanges.ToPointer(), 6, 6);
            icons_config.GlyphRanges = (IntPtr)(ushort*)Fontaudio.IconFontRanges.ToPointer();

            if (TryGetEmbeddedResourceBytes("fontaudio.ttf", out var fontDataBuffer))
            {
                fixed (byte* buffer = fontDataBuffer)
                {
                    Fontaudio.IconFontPtr = ImGui.GetIO().Fonts.AddFontFromMemoryTTF(new IntPtr(buffer), fontDataBuffer.Length, fontSize, icons_config, Fontaudio.IconFontRanges);
                }
            }
            else
                throw new Exception("Couldn't load fontaudio.ttf font");
        }
    }

    public static IntPtr LoadImage(string embeddedResourceName)
    {
        TryGetEmbeddedResourceBytes(embeddedResourceName, out var imgData);
        ...
        return Program._controller.GetOrCreateImGuiBinding(Program._gd.ResourceFactory, dimg);
    }
